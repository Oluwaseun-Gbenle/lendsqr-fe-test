...
import { UserDetailsProps, UsersBankDetailsProps } from '../../Interfaces/UsersDetails';
import { fetchUserBankDetails, fetchUserDetails } from '../../data services/apiservice';
...
const UsersDetails: FC = () => {
    ...
    const [userBankDetails, setuserBankDetails] = useState<UsersBankDetailsProps>();
    ...
    useEffect(() => {
        const getUserBankDetails = async () => {
            try {
                ...
                const storedBankData = localStorage.getItem("storedUserBankDetails")
                setuserBankDetails(JSON.parse(storedBankData || ""));
            } catch (error) {
        ...
        const getUserDetails = async () => {
            try {
                ...
                const storedUserData = localStorage.getItem("storedUserDetails")
    ...
    return (
        <div className='users-container'>
            <div className="page-title">
                <div className="go-back-text" onClick={() => { navigate(-1) }} >{backSVG} <span>Back to Users</span></div>
            ...
            <div className="user-detail-card">
                <div className="user-details-display">
                    ...
                    <div className="user-tier">
                        ...
                        {[...Array(3 - (userBankDetails?.stars || 0))].map((_, i) => (
                            (userBankDetails?.stars || 0) < 3 && <span key={i} className="star-icon">{starOutlineSVG}</span>
